  ...
  return (
    <Card className="overflow-hidden border-border/60 p-4 md:p-6">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-4 md:gap-6">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="truncate font-display text-2xl md:text-3xl">{profile.fullName}</h2>
            <img src={profile.countryFlagUrl} alt={profile.countryName} className="h-5 w-7 rounded-sm object-cover" />
            <Badge variant="secondary" className="rounded-md">{profile.teamName}</Badge>
          </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 border-t pt-4 sm:grid-cols-3 md:grid-cols-5">
        ...
        <Stat label="Age" value={`${profile.age} (${new Date(profile.birthDate).toLocaleDateString("en-US", { day: "2-digit", month: "short", year: "numeric" })})`} />
